...
class Simple_login {
    ...
    * @param string password dari input form*/
    public function login($username, $password) {
        ...
        $query = $this->CI->db->get_where('user_cust',array('username'=>$username,'password' =>md5($password)));
        if($query->num_rows() == 1) {
            //ambil data user berdasar username
            $row = $this->CI->db->query('SELECT id_user FROM user_cust where username = "'.$username.'"');
            $admin = $row->row();
            $id = $admin->id_user;
            //set session user
            ...
            redirect(site_url('frontend/page'));
        }else{
